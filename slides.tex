...
\item The GO project has $\sim$ 44,400 validated terms  \hyperlink{aphylo-goexample}{\beamergotobutton{more}}, $\sim$ 7.9M annotations on $\sim$ 4,600 species.
\item About $\sim$ 550,000 are on human genes.
\item Roughly half of human genes ($\sim$ 10,000 / 20,000) have some
form of annotation.
\item We know something of less than 10\% of known genes (near 1.7M). % 40610
...
\begin{figure}
	...
	\mode<beamer>{
	\only<1>{\input{fig/phylo-model-0.pdf_tex}}\only<2>{\input{fig/phylo-model-0b.pdf_tex}}\only<3>{\input{fig/phylo-model-0c.pdf_tex}}\only<4>{\input{fig/phylo-model-1.pdf_tex}}\only<5-6>{\input{fig/phylo-model-2a.pdf_tex}}\only<7>{\input{fig/phylo-model-2b.pdf_tex}}\only<8>{\input{fig/phylo-model.pdf_tex}}
...
\begin{minipage}[m]{.38\linewidth}
	...
	\begin{itemize}
		...
		\item Only a fraction of annotations is known.
	\end{itemize}
\end{minipage}
...
\begin{frame}[c,label=phylo-old-results]
	...
	\byside{High Accuracy}{Matches state-of-the-art phylo-based prediction models with $\sim$ 0.75 AUC}
\end{frame}
...
% ------------------------------------------------------------------------------
\begin{frame}[c]
	\frametitle{Phylogenetics Modeling: Pooled-data models}
...
\begin{table}
	...
	\begin{tabular}{lm{.15\linewidth}*{3}{m{.15\linewidth}<\centering}}\toprule
		...
		& & Fixed to Beliefs & Empirical & Free \\ \midrule
		\multirow{3}{*}[-.25cm]{\rotatebox{90}{Model Parameters}} %
		& Shared across~trees             & \hlc{2-}{ } & - 	& - \\ \cmidrule(r){2-5}
		& Class \hphantom{.....} specific & \hlc{3-}{ }  & \hlc{4-}{ } & \hlc{4-}{ } \\ \cmidrule(r){2-5}
		& Function \hphantom{..} specific & -           & \hlc{5-}{ } & \hlc{5-}{ } \\
		\bottomrule
	\end{tabular}
	\caption{Ways to fit pooled-data models.}
\end{table}
\end{minipage}
\begin{minipage}[m]{.38\linewidth}
\begin{enumerate}
	\item<2-> Featured in the first version of the model.
	\item<3-> (new) Improves accuracy (sometimes).
	\item<4-> (new) Hard to fit and no significant improvement (yet).
	\item<5-> To-be-attempted.
\end{enumerate}
\end{minipage}
...
% ------------------------------------------------------------------------------
\section{Exponential Random Graph Models for Small Networks}
% \frame{\frametitle{Contents}\tableofcontents}
...
\begin{frame}[t,label=discrete-exponential]
	...
	\begin{minipage}[m]{.55\linewidth}
		...
		\begin{itemize}
			...
			\item<3-> Focused on sufficient statistics $\s{\graph,x}$ \uncover<4->{\textbf{e.g.}\\Two Bernoulli, a.k.a. Erd\H{o}s-R\'enyi, random graphs}.
			\item<5-> Most (all) applications use \textbf{approximations}.
		\end{itemize}
	\end{minipage}\hfill
	\begin{minipage}[m]{.44\linewidth}
		\begin{figure}[!htbp]
			\centering
			\includegraphics<4->[width=.9\linewidth,clip,trim={.5cm .5cm .5cm .5cm}]{ring-and-kite.pdf}
...
\hfill
	\begin{minipage}[t]{.6\linewidth}
		...
		\end{figure}
		\only{\Large Is Gender Homophily \textit{a} feature of these graphs?}
		}
	\end{minipage}
...
\hlcAlt{4}{Homophily (gender)}                        & $-0.03$       & $-0.01$       & $-0.20$       & $-0.12$       & $-0.01$       & $-0.12$       \\
				...
				&               &               & $(1.32)$      &               &               &               \\
				\hlcAlt{4}{Sender (female)}                           &               &               &               & $0.46^{*}$    &               & $0.46^{*}$    \\
				&               &               &               & $(0.18)$      &               & $(0.18)$      \\
...
\begin{frame}[c]
	...
	\begin{itemize}
		\item<2-> High predictive power (Term A $\perp$ Term B):
		\begin{itemize}
			\item<3-> Stepped slope.
			\item<3-> Narrow Confidence Interval (CI).
		\end{itemize}
		\item<4-> Low predictive power (Term A $\perp$ Term C):
		\begin{itemize}
...
\begin{figure}
	...
	\mode<beamer>{\only<1-2>{\input{fig/conditional-prob-ttriad-1.pdf_tex}}\only<3->{\input{fig/conditional-prob-ttriad.pdf_tex}}}
...
\section{Connecting the dots: Phylogenetic Modeling with ERGMs}
...
\begin{frame}[c]
	...
	\begin{minipage}[m]{.59\linewidth}
		In a model with 3 functions and 2 offspring per node
		\begin{itemize}
			\item Full Markov transition matrix: $2^3 \times 2^6 = 512$
			\item Using sufficient statistics:
			\begin{itemize}
				\item[] Pairwise co-evolution: 3 terms,
				\item[]	Pairwise Neofunctionalization: 3 terms,
				\item[] Pairwise Subfunctionalization: 3 terms,
				\item[] Function specific gain: 3 terms,
				\item[] Function specific loss: 3 terms,
				\item[Total:] 15 parameters. 
			\end{itemize}
		\end{itemize}
	\end{minipage}\hfill
...
\begin{minipage}[m]{.65\linewidth}
	...
	\begin{minipage}[m]{.49\linewidth}
		\uncover<6->{\includegraphics[width=.99\linewidth]{barray-phylo.png}}
	\end{minipage}
	\begin{minipage}[m]{.49\linewidth}
		\uncover<7->{\includegraphics[width=.99\linewidth]{barray-network.png}}
	\end{minipage}
	\uncover<6->{\caption{Screenshots from the project's website on GitHub.}}
	\end{figure}
\end{minipage}
\end{frame}
...
\newcommand{\ultima}[3]{\begin{minipage}[t]{#1\linewidth}%
		\begin{center}\shadowbox{#2}\end{center}\vspace{-.75cm}\hfill%	
...
\ultima{.3}{After my dissertation}{%
	...
	\textbf{ERGMs}
	\begin{itemize}
		\item Small networks are cool again.
		\item Exact $=$ Explosion of methods
	\end{itemize}\pause
}\hfill
\ultima{.39}{Products}{
\textbf{Publications}
