...
\item Some times, the cannot understand a process unless we look at it as a whole.
\item There's a reason why we usually assume {\it IID}.
\item {\it Modern} (as of today) computational tools help us cope with that.
\end{itemize}
\end{frame}
...
\begin{minipage}[m]{.38\linewidth}
	...
	\begin{itemize}
		...
		\item Only a fraction of the genes has some form of annotation.
	\end{itemize}
\end{minipage}
...
\begin{minipage}[m]{.3\linewidth}
	\begin{itemize}
		\item<2-> If COVID-19 has taught us something is that networks matter.
		\item<3-> And specially small networks: Families, teams, friends, etc. \uncover<4->{The cornerstone of larger social systems.}
...
\begin{frame}[t,label=discrete-exponential]
	...
	\begin{minipage}[m]{.55\linewidth}
		...
		\begin{itemize}
			...
			\item<4-> For any array of size $(n\times m)$, there are $2^{(n\times m)}$ possible arrangements.
		\end{itemize}
	 \uncover<5->{\LARGE Most (all) applications use \textbf{approximations}.\normalsize}
	\end{minipage}\hfill
	\begin{minipage}[m]{.44\linewidth}
		\begin{figure}[!htbp]
			\centering
			\uncover<4->{\includegraphics[width=.9\linewidth,clip,trim={.5cm .5cm .5cm .5cm}]{ring-and-kite.pdf}}
	...
	\begin{table}
		...
		\scalebox{.65}{
			\begin{tabular}{l c c c c c c }
				\toprule
				& (1) & (2) & (3) & \hlcAlt{6}{(4)} & (5) & \hlcAlt{6}{(4b)} \\
				\midrule
...
\hlcAlt{5}{Homophily (gender)}                        & $-0.03$       & $-0.01$       & $-0.20$       & $-0.12$       & $-0.01$       & $-0.12$       \\
				...
				&               &               & $(1.32)$      &               &               &               \\
				\hlcAlt{5}{Sender (female)}                           &               &               &               & $0.46^{*}$    &               & $0.46^{*}$    \\
				&               &               &               & $(0.18)$      &               & $(0.18)$      \\
				...
				N replicates                              &               &               &               &               &               & \hlcAlt{6}{1000}          \\ \bottomrule
...
\begin{minipage}[c]{.28\linewidth}
	...
	\item \alert<5>{No support for gender homophily, but evidence of females sending more ties.}
	\item \alert<6>{Each 1,000 bootstrap replicates took roughly 0.08 secs.}
	\end{enumerate}
\end{minipage}
	
	\vfill\hfill\hyperlink{example-data-ergm}{\beamergotobutton{data}}
...
\begin{frame}[label=example-data-ergm]
	...
	\end{table}
\vfill\hfill\hyperlink{ergmito-table}{\beamerreturnbutton{go back}}
\end{frame}
